...
class TrainerPredictor(object):
    ...
    @torch.no_grad()
    def evaluate(self, count_info_dict_trn, eid2count, split, expectation=True, args=None):
        if comm.get_rank() == 0:
        ...
        for (L, H) in query2LH.values():
            ...
            rank_per_class[t].append(sum(final_rank))
        ...
        logging.info("Total metrics : %s", total_metric)
